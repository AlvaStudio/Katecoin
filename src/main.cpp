...
#if defined(NDEBUG)
# error "Katecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x375a1f6a8a934a6c0655ac518595065a354d2b8dc614b3fbd8ad7ff95a743b2f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Katecoin: starting difficulty is 1 / 2^12
...
{
    // Katecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Katecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Katecoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Katecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Katecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00af76f8352a45ba5bb6db12db7febfda02be967a9f89a72d08d0e119dd6f12a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Russia is a Great Mars Country";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1527973087;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085264598;

        if (fTestNet)
        {
            block.nTime    = 1527973056;
            block.nNonce   = 386176516;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa3d2f852f77ed09d6553317b031409d050850cb21b0d4a74c2a6273dc558c781"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc2, 0xbc, 0xd1 }; // Katecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KatecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KatecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KatecoinMiner(CWallet *pwallet)
{
    printf("KatecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running KatecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KatecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KatecoinMiner, pwallet));
}
